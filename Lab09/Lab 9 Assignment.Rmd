...
ens_presCovs = extract(ensemble, vathPresXy)
ens_presCovs
ens_presCovs = data.frame(vathPresXy, ens_presCovs, pres=1)
ens_presCovs
ens_presCovs = ens_presCovs[complete.cases(ens_presCovs),]
ens_presCovs

ens_valCovs = extract(ensemble, vathValXy)
ens_valCovs = data.frame(vathValXy, ens_valCovs)

ens_backcov = extract(ensemble, backXy)
ens_backcov
ens_backcov = data.frame(backXy, ens_backcov, pres=0)
ens_backcov
ens_backcov = ens_backcov[complete.cases(ens_backcov),]
ens_backcov

ens_backcov = ens_backcov %>% select(-ID)
colnames(ens_presCovs)[1:2] = c('x', 'y')

ens_presBackCovs = rbind(ens_presCovs, ens_backcov)
ens_presBackCovs

# discrimination stats data frame setup
tmp = ens_valCovs %>% mutate(VATH = vathVal$VATH)
tmp = tmp[complete.cases(tmp),]


ens_valData = data.frame('ID' = 1:nrow(tmp)) %>% 
  mutate(obs = tmp$VATH,
         ens_valCovs)
